...
void on_client_closed(uv_handle_t* handle)
{
    ...
    /*
    The client handle owns the peer state storing its
    address in the data field, so we free it here.
    */
   if(client->data){
       free(client->data);
   }
...
void on_peer_read(uv_stream_t* client, ssize_t nread, const uv_buf_t* buf)
{
    if(nread < 0){
        ...
        // From the documentation of uv_read_cb: 
        // nread might be 0, which does not
        // indicate an error or EOF. 
        // This is equivalent to EAGAIN or EWOULDBLOCK
        // under read(2).
    }
    else{
        // nread > 0
        ...
        if(peerstate->state == INITIAL_ACK){
            // If the initial ACK hasn't been sent
            // for some reason,
            // ignore whatever the client sends in
            free(buf->base);
        }
        ...
        if(peerstate->sendbuf_end > 0){
            ...
            //The write buffer will point to the buffer
            // stored in the peer state for thie client
            uv_buf_t writebuf = 
                uv_buf_init(peerstate->sendbuf, 
                peerstate->sendbuf_end);
            uv_write_t* writereq = (uv_write_t*)xmalloc(
                sizeof(*writereq)
            );
            writereq->data = peerstate;
            int rc;
            if((rc = uv_write(writereq,
                (uv_stream_t*)client,
                &writebuf,1, on_wrote_buf)) < 0){
...
/*
Then we know for sure that the initial '*' 
was sent to the peer, we start listening to 
incoming data from this peer 
by calling uv_read_start, 
which registers a callback (on_peer_read) 
that will be invoked 
by the event loop whenever 
new data is received on the socket from the client:
*/
...
void on_write_init_ack(uv_write_t* req, int status)
{
    ...
    peer_state_t* peerstate = (peer_state_t*)req->data;
    // Flip the peer state to WAIT_FOR_MSG, 
    // and start listening for incoming data
    // from this peer.
    peerstate->state = WAIT_FOR_MSG,
    peerstate->sendbuf_end = 0;

    int rc;
    if((rc = uv_read_start((uv_stream_t*)peerstate->client,
        on_alloc_buffer, on_peer_read)) < 0){

        die("uv_read_start failed: %s", uv_strerror(rc));
    }

    // Note: the write request doesn't 
    // own the peer state, 
...
void on_peer_connected(uv_stream_t* server_stream, int status)
{
    ...
    // client will represent this peer; 
    // it's allocated on the heap and only
    // released when the client disconnects. 
    // The client holds a pointer to
    // peer_state_t in its data field; 
    // this peer state tracks the protocol state
    ...
    if(uv_accept(server_stream,(uv_stream_t*)client)==0){
        ...
        //ENqueue the write request to send the ack
        //when it's done,
        //on_write_init_ack will called.
        //The peer state is passed to the write
        //request via the data pointer,
        //the write request does not own this peer state
        //it's owned by the client handle
...
int main(int argc, char** argv)
{
    ...
    if((rc = uv_ip4_addr("0.0.0.0", portnum, &server_address)) < 0){
        die("uv_ipv4_addr failed: %s", uv_strerror(rc));
    }
    ...
    //Listen on the socket for new peers to connect.
